...
#     rand_points <- suppressWarnings(irs(p_malla, 
#                        n_base = n_target, 
#                        n_over = n_reserva,
...
#         points_bined <- bind_rows(rand_points$sites_base,
#                                   rand_points$sites_legacy,
#                                   rand_points$sites_over)
#     } else {
#       points_bined <- bind_rows(rand_points$sites_base,
#                                   rand_points$sites_over) 
#     }
...
if (n_punts >= n_target + n_reserva) {
   ...
   # SI hi ha punts mostrejats 
    if (legacy == TRUE) {
      ...
      #   rand_bal_clean <- rand_bal[!is.na(rand_bal)]
      #   n_fail <- sum(is.na(rand_bal))          

      # p_val <- (sum(rand_bal_clean >= obs_bal) + 1) / (length(rand_bal_clean) + 1)
      # ses <- (points_bal$value - mean(rand_bal_clean))/sd(rand_bal_clean)
      # mean_rand <- mean(rand_bal_clean)
...
log_msg(level = paste(hic, regio), msg = paste(hic, regio, legacy, method, round(points_bal$value, 5), dist_sup_200, path, n_punts))

n_legacy <- if (!is.null(legacy_points)) nrow(legacy_points) else 0

meta_results <- c(hic, regio, legacy, n_legacy, method, round(points_bal$value,6), dist_sup_200, path, n_punts)
names(meta_results) <- c("HIC", "Regio", "Legacy", "N_legacy", "Metode", "sp_balance_grts", "Dist_sup_200", "Ruta", "N_punts_disp") 
...
meta_results <- c(hic, regio, legacy, n_legacy, method, dist_sup_200, path, nrow(points_sample))
names(meta_results) <- c("HIC", "Regio", "Legacy", "N_legacy", "Metode", "Dist_sup_200", "Ruta", "N_punts_disp") 
